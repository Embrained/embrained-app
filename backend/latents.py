...
class LatentGenerator:
    ...
    def generate_latents_from_file(self, filename):
        ...
        if isinstance(data, list):
            ...
            elif 'actions' in first and isinstance(first['actions'], list):
                # Episodes list (episodes.json)
                # Each episode has actions, start_frame, goal_frame
                for ep in data:
                    # Include start/goal? usually they are part of actions or separate
                    # episodes.json from training.py: start_frame, goal_frame, actions
                    # actions is a list of transitions
                    if 'start_frame' in ep:
                        image_paths.append(ep['start_frame']['image_path'])
                    for action in ep['actions']:
                        image_paths.append(action['image_path'])
                    # Goal frame logic if needed? usually goal is the last action's next state
                    # But let's just grab what's available
                    if 'goal_frame' in ep:
                         image_paths.append(ep['goal_frame']['image_path'])
