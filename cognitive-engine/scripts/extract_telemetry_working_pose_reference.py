...
class TelemetryExtractor:
    ...
    def _extract_pure_blobs(self, d_path):
        ...
        sample_rows = df.sample(n=min(20, len(df)))
        frames = []
        ...
        local_bg = np.median(frames, axis=0).astype(np.uint8)
    ...
    def calibrate(self):
        ...
        print(f"Calibration Complete. Arena Bounds: {self.arena_bounds}")
        return all_blobs
    ...
    def load_cache(self, path):
        ...
        if os.path.exists(path):
             ...
             self.arena_bounds = data['arena_bounds'].item()
             self.median_bg = data['median_bg']
             return True
        return False
