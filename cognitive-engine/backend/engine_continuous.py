...
from modules.continuous_exploration import ContinuousExplorationSystem, ContinuousWASD
...
class ContinuousCognitiveEngine:
    def __init__(self, dry_run=False, robot_ip=None, stream_port=81, rec_prefix='markov', use_webcam=False, is_simulation=False):
        ...
        try:
            ...
            self.explorer = ContinuousExplorationSystem() 
            self.cql_controller = ContinuousWASD() # NEW CQL PACER
        ...
        if found_path and os.path.exists(found_path):
            ...
            try:
                ...
                if "telemetry_oracle" in model_filename or "algorithmic_oracle" in model_filename:
                    ...
                    current_cve = self.state.get('cve_model', '')
                    if cve_candidate != current_cve:
                         logging.debug(f"Auto-switching VAE to {cve_candidate} for policy {model_filename}")
                         success = self.load_cve_model(cve_candidate)
                         if not success:
                             logging.warning(f"Could not auto-load VAE {cve_candidate}. Manifold might be mismatched.")
                         if hasattr(self, 'vision') and self.vision:
            ...
            if self.planner and self.planner.load_model(found_path):
                ...
                self.planner.goals = []
                if hasattr(self, 'planner') and hasattr(self.planner, 'latent_buffer'):
            ...
            try:
                ...
                if img is not None:
                    ...
                    last_goal_idx = self._update_state(
                        ...
                        reflex_triggered=reflex_triggered, is_bout_start=is_bout_start, ui_img=display_img, webcam_ui_img=annotated_webcam_img, active_controller_str=ctrl_str, last_action_sent=last_action_sent
                    )
