...
class ForwardNetwork(nn.Module):
    """Predicts the next future latent state using an RNN for contextual inertia."""
    def __init__(self, latent_dim=32, action_dim=3, action_embed_dim=8):
        ...
        # Temporal memory (expects [batch_size, seq_len, latent_dim])
    ...
    def forward(self, l_seq, a1):
        # l_seq represents [l_prev, l_current] (shape: batch x 2 x dim)
        rnn_out, _ = self.rnn(l_seq)
        
        # Extract the hidden status at the latest time step
        last_hidden = rnn_out[:, -1, :]
        a_emb = self.action_embed(a1)
        x = torch.cat([last_hidden, a_emb], dim=-1)
        
        # Residual added to the latest latent
...
class TopologicalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while action_A == -1:
            ...
            # 2. Pick node A (require index >= 1 to get previous step)
            idx_A = random.randint(1, t_len - 2)
            node_A = traj[idx_A]
        ...
        node_A_prev = traj[idx_A - 1]
        path_a_prev = node_A_prev.get('image_path', '')
        latent_A_prev = self.latents[path_a_prev]
        ...
        return latent_A_prev, latent_A, latent_A_Next, latent_B, temporal_dist, torch.tensor(action_A, dtype=torch.long)
...
def train(data_root, num_epochs=20, batch_size=128, learning_rate=1e-4, stop_event=None, progress_callback=None, model_filename=None):
    ...
    print("\n--- Initiating Dual-Module Topological Training (Forward Dynamics: Temporal RNN) ---")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for lA_prev, lA, lA_next, lB, true_dist, true_act in dataloader:
            lA_prev, lA, lA_next, lB = lA_prev.to(device), lA.to(device), lA_next.to(device), lB.to(device)
            true_dist = true_dist.to(device)
            true_act = true_act.to(device)
            
            # Form sequence for RNN: [batch, seq=2, params]
            l_seq = torch.stack([lA_prev, lA], dim=1)
            
            # Module A: Distance Prediction
            pred_dist = temporal_net(lA, lB)
            loss_t = mse_loss(pred_dist, true_dist)
            
            # Module B: Forward Dynamics Prediction (Sequence passed to RNN)
            pred_lA_next = forward_net(l_seq, true_act)
            loss_f = mse_loss(pred_lA_next, lA_next)
    ...
    print(f"\nTopological SLAM Assembly Complete. Best Forward MSE: {best_loss_f:.4f}")
