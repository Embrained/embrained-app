...
def train(data_root, num_epochs=20, batch_size=128, learning_rate=1e-4, stop_event=None, progress_callback=None, model_filename=None):
    ...
    if os.path.exists(cache_path):
        ...
        print(f"Cache not found. Initializing empty latent map.")
        latent_dict = {}

    print("Verifying latent cache mapping across all trajectories...")
    missing_paths = set()
    for traj in trajectories:
        for node in traj:
            p = node.get('image_path', '')
            if p not in latent_dict and p:
                missing_paths.add(p)
                
    if missing_paths:
        print(f"Discovered {len(missing_paths)} unseen dataset frames. Extracting representations dynamically...")
        with torch.no_grad():
            for i, p in enumerate(list(missing_paths)):
                ...
                if os.path.exists(img_path):
                    try:
                        ...
                        _, mu, _ = vae(t_img)
                        latent_dict[p] = mu.cpu()
                    except Exception as e:
                        print(f"Extraction failed for {p}: {e}")
                if i % 1000 == 0 and i > 0: print(f"  Extracted {i} / {len(missing_paths)}...")
                
        print(f"Updating global cache...")
        torch.save({"path_map": latent_dict}, cache_path)
    ...
    print("\n--- Initiating Dual-Module Topological Training (Forward Dynamics: MSE) ---")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for lA, lA_next, lB, true_dist, true_act in dataloader:
            ...
            # Module B: Forward Dynamics Prediction (Standard MSE Loss)
