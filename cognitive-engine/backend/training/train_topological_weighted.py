...
def train(data_root, num_epochs=20, batch_size=128, learning_rate=1e-4, stop_event=None, progress_callback=None, model_filename=None):
    ...
    print("\n--- Initiating Dual-Module Topological Training (Forward Dynamics: Action-Weighted MSE) ---")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for lA, lA_next, lB, true_dist, true_act in dataloader:
            ...
            # Module B: Forward Dynamics Prediction (Action-Weighted MSE Loss)
            pred_lA_next = forward_net(lA, true_act)
            
            # Compute unreduced MSE
            loss_fs = F.mse_loss(pred_lA_next, lA_next, reduction='none')
            loss_fs = loss_fs.mean(dim=1) # average over the latent dimensions
            
            # Apply class-specific weights: FWD=2.0 (Act 0), ROT=0.5 (Act 1, 2)
            weights = torch.ones_like(true_act, dtype=torch.float)
            weights[true_act == 0] = 2.0
            weights[true_act == 1] = 0.5
            weights[true_act == 2] = 0.5
            
            loss_f = (loss_fs * weights).mean()
