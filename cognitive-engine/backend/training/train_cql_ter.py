...
from config import DATA_DIR, MODELS_DIR, HIDDEN_DIM, ACTION_DIM
...
class TERDataset(Dataset):
    def __init__(self, data_root, vae_path, device, gamma=0.9):
    ...
    def _build_graph_and_targets(self):
        ...
        node_id_map = {} # Maps unique frame paths to IDs
        nodes_data = {}
        ...
        for s, seq in sessions.items():
            ...
            for i in range(len(seq) - 1):
                ...
                # We use the image path as the unique state identifier
                G.add_edge(u_path, v_path, action=u.get('macro_action', 0))
                nodes_data[u_path] = u
                nodes_data[v_path] = v

        print(f"Graph built: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges.")
        
        # Identify goal states (For simplicity, pick 1 random node as the goal, or the targets)
        # We will use the TARGET_IMAGES as terminal states
        ...
        # BFS Backward from terminal nodes
        # We want to find the shortest path from every node TO a terminal node.
        # This means BFS on the REVERSED graph.
        R = G.reverse()
        ...
        max_dist = max([d for d in distances.values() if d != float('inf')] + [100])
        
        for u, v, data in G.edges(data=True):
            if distances[u] == float('inf'): continue
            
            # True V value
            # Reward is 1 at goal, 0 otherwise
            # V(s) = gamma^steps
            v_target = (self.gamma ** distances[u])
            
            action = int(data.get('action', 0))
            if action not in [1, 2, 3, 4]: continue
    ...
    def __getitem__(self, idx):
        ...
        target = torch.tensor(sample['v_target'], dtype=torch.float)
        
        return state_curr, action, target

def main():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Starting Topological Experience Replay (TER) Training on {device}")
    ...
    dataset = TERDataset(DATA_ROOT, VAE_PATH, device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (state, action, target) in enumerate(dataloader):
    ...
    print(f"✅ Saved trained TER Policy to {out_path}")
